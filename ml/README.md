...
Каталог содержит конфигурацию и скрипты для работы с тремя LLM: загрузка/проверка доступности, бенчмарки и использование в коде. Backend не изменяется — подключаем только его клиенты (GigaChat, Cotype, T-Pro) для тестов и бенчмарков.
