...
Скрипт `llm_benchmark.py` гоняет запросы к моделям и собирает метрики: время ответа, оценка качества, доля успешных ответов, ориентировочная стоимость. Используются либо три HF-модели (Qwen, Llama, T-Pro), либо клиенты backend (флаг `--backend`). Backend не изменяется.
...
- **--backend** — бенчмарк по клиентам backend (GigaChat, Cotype, T-Pro); нужна доступная папка `backend/`.
...
**Сравнение оптимизации (ML-5):** для сравнения результатов оптимизации по всем трём моделям и генерации отчёта:
