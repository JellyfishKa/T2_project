  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
      <div class="flex items-center justify-between">
        <div>
          <h3 class="text-lg font-semibold text-gray-900">Загрузка магазинов из файла</h3>
    ...
    <div class="px-6 py-5">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2">
            <p class="text-sm font-medium text-gray-900">
              <span class="text-blue-600">Нажмите для выбора файла</span> или перетащите его сюда
            </p>
      ...
      <div v-if="previewData.length > 0" class="mt-6">
        <div class="flex items-center justify-between mb-4">
          <h4 class="text-sm font-medium text-gray-900">Предпросмотр данных (первые 5 строк)</h4>
          <span class="text-sm text-gray-500">{{ previewData.length }} строк</span>
        </div>
        ...
        <div class="mt-4 text-sm text-gray-500">
          Обнаружено {{ previewData.length }} магазинов. {{ validationErrors.length > 0 ? `Найдено ${validationErrors.length} ошибок.` : 'Все данные корректны.' }}
        </div>
        ...
        <div v-if="validationErrors.length > 0" class="mt-4">
          <div class="rounded-md bg-yellow-50 p-4">
            <div class="flex">
              ...
              <div class="ml-3">
                <h3 class="text-sm font-medium text-yellow-800">Обнаружены ошибки в данных</h3>
      ...
      <div v-if="uploadedLocations.length > 0" class="mt-8">
        <div class="flex items-center justify-between mb-4">
          <h4 class="text-sm font-medium text-gray-900">Загруженные магазины</h4>
          <span class="text-sm text-gray-500">{{ uploadedLocations.length }} магазинов</span>
        </div>
          ...
          >
            <div class="flex items-center">
              ...
              <div class="ml-3">
                ...
                <p class="text-xs text-gray-500">{{ location.city }}, {{ location.street }}, д. {{ location.houseNumber }}</p>
              </div>
            </div>
            <button
...
const validateData = (data: FilePreviewRow[]) => {
  ...
  data.forEach((row, index) => {
    ...
    const requiredFields = ['name', 'city', 'street', 'houseNumber', 'latitude', 'longitude']
    
    requiredFields.forEach(field => {
      if (!row[field] || String(row[field]).trim() === '') {
        validationErrors.value.push(`Строка ${rowNumber}: Отсутствует обязательное поле "${field}"`)
      }
    })
    ...
    if (row.latitude) {
      ...
      if (isNaN(lat) || lat < -90 || lat > 90) {
        validationErrors.value.push(`Строка ${rowNumber}: Некорректная широта "${row.latitude}"`)
      }
    }
    ...
    if (row.longitude) {
      ...
      if (isNaN(lon) || lon < -180 || lon > 180) {
        validationErrors.value.push(`Строка ${rowNumber}: Некорректная долгота "${row.longitude}"`)
      }
    }
    ...
    if (row.houseNumber) {
      ...
      if (!/^[1-9]\d*[а-яА-Я]?(\/\d+)?$/.test(houseNum)) {
        validationErrors.value.push(`Строка ${rowNumber}: Некорректный номер дома "${houseNum}"`)
      }
    }
  })
}
