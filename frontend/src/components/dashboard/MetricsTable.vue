<template>
  <div class="overflow-hidden">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="metric in metrics" :key="metric.id">
            <td class="px-3 py-4 whitespace-nowrap">
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" :class="getModelBadgeClass(metric.model)">
              ...
              <div class="flex items-center">
                <div class="w-24 bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    :style="{ width: getResponseTimePercentage(metric.response_time_ms) + '%' }"
                  ></div>
                </div>
                <span class="ml-3 text-sm text-gray-900">{{ metric.response_time_ms }} мс</span>
              </div>
            </td>
            <td class="px-3 py-4 whitespace-nowrap">
              <div class="flex items-center">
                ...
                <span class="ml-3 text-sm text-gray-900">{{ (metric.quality_score * 100).toFixed(1) }}%</span>
              </div>
            </td>
            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-900">
    ...
    <div class="sm:hidden space-y-3">
      <div v-for="metric in metrics" :key="metric.id" class="bg-white border border-gray-200 rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" :class="getModelBadgeClass(metric.model)">
            {{ getModelName(metric.model) }}
          </span>
          <span class="text-xs text-gray-500">{{ formatTime(metric.timestamp) }}</span>
        </div>
        
        <div class="space-y-2">
          <div class="flex justify-between">
            <span class="text-sm text-gray-600">Маршрут:</span>
            <span class="text-sm font-medium text-gray-900">{{ metric.route_id }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-sm text-gray-600">Время ответа:</span>
            <span class="text-sm font-medium text-gray-900">{{ metric.response_time_ms }} мс</span>
          </div>
          <div class="flex justify-between">
            <span class="text-sm text-gray-600">Качество:</span>
            <span class="text-sm font-medium text-gray-900">{{ (metric.quality_score * 100).toFixed(1) }}%</span>
          </div>
          <div class="flex justify-between">
            <span class="text-sm text-gray-600">Стоимость:</span>
            <span class="text-sm font-medium text-gray-900">{{ metric.cost_rub.toFixed(2) }} ₽</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="metrics.length === 0" class="text-center py-8">
      ...
      <p class="mt-1 text-sm text-gray-500">Запустите оптимизацию для создания метрик</p>
    </div>
  </div>
</template>
