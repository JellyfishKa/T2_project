<template>
  <div class="space-y-6">
    ...
    <div v-if="route" class="bg-gray-50 rounded-lg p-4">
      <div class="flex items-center justify-between">
        ...
        <div class="text-right">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium" :class="getModelBadgeClass(route.model_used)">
            {{ getModelName(route.model_used) }}
          </span>
          <p class="mt-1 text-sm text-gray-600">{{ formatDate(route.created_at) }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="metrics.length > 0" class="border-t border-gray-200 pt-6">
      <h5 class="text-lg font-medium text-gray-900 mb-4">Производительность моделей для этого маршрута</h5>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="metric in metrics" :key="metric.id">
              <td class="px-4 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium" :class="getModelBadgeClass(metric.model)">
    ...
    <div v-else class="text-center py-8 border rounded-lg border-gray-200">
      ...
      <p class="mt-1 text-sm text-gray-500">Выберите маршрут для просмотра метрик</p>
    </div>
  </div>
</template>
