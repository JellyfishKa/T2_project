<template>
  <div class="space-y-4">
    ...
    <div v-if="isLoading" class="text-center py-4">
      ...
      <p class="mt-2 text-sm text-gray-600">Загрузка результатов бенчмарка...</p>
    </div>
    ...
    <div v-else-if="benchmarkResults.length > 0" class="space-y-6">
      ...
      <div>
        <h4 class="text-sm font-medium text-gray-900 mb-3">Время ответа (мс)</h4>
        <div class="space-y-3">
          <div v-for="result in benchmarkResults" :key="result.model" class="flex items-center">
            <div class="w-24 text-sm font-medium text-gray-900">{{ getModelName(result.model) }}</div>
            <div class="flex-1">
              <div class="flex items-center">
                <div class="w-full bg-gray-200 rounded-full h-3">
                  <div 
                    ...
                    :style="{ width: getResponseTimeWidth(result.avg_response_time_ms) + '%' }"
                  ></div>
                </div>
              ...
              <div class="text-xs text-gray-500 mt-1">
                Min: {{ result.min_response_time_ms }} | Max: {{ result.max_response_time_ms }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        <h4 class="text-sm font-medium text-gray-900 mb-3">Качество решения (%)</h4>
        <div class="space-y-3">
          <div v-for="result in benchmarkResults" :key="result.model" class="flex items-center">
            <div class="w-24 text-sm font-medium text-gray-900">{{ getModelName(result.model) }}</div>
            <div class="flex-1">
              ...
              <div class="flex items-center">
                <div class="w-full bg-gray-200 rounded-full h-3">
                  <div 
                    ...
                    :style="{ width: getCostWidth(result.total_cost_rub) + '%' }"
                  ></div>
                </div>
                <div class="ml-3 text-sm font-medium text-gray-900 w-16 text-right">
      ...
      <!-- Success Rate -->
      <div>
        <h4 class="text-sm font-medium text-gray-900 mb-3">Успешность выполнения</h4>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div v-for="result in benchmarkResults" :key="result.model" class="bg-gray-50 rounded-lg p-3">
            <div class="flex items-center justify-between">
              <span class="text-sm font-medium text-gray-900">{{ getModelName(result.model) }}</span>
              <span class="text-sm font-semibold" :class="getSuccessRateColor(result.success_rate)">
    ...
    <div v-else class="text-center py-8 border rounded-lg border-gray-200">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900">Нет результатов бенчмарка</h3>
      <p class="mt-1 text-sm text-gray-500">Запустите бенчмарк для сравнения моделей</p>
    </div>
  </div>
</template>
