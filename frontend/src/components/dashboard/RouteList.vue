<template>
  <div class="overflow-hidden">
                ...
                <td class="px-4 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-10 w-10">
                      <div class="h-10 w-10 rounded-lg flex items-center justify-center" :class="getModelColor(route.model_used)">
                        <span class="text-white font-bold text-sm">{{ getModelInitial(route.model_used) }}</span>
                      </div>
                    </div>
                    <div class="ml-4">
                      <div class="text-sm font-medium text-gray-900">{{ route.name }}</div>
                      <div class="text-sm text-gray-500">{{ formatDate(route.created_at) }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-4 py-4 whitespace-nowrap">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="getModelBadgeClass(route.model_used)">
    ...
    <div class="sm:hidden space-y-3">
      <div 
        ...
        :class="[
          ...
          selectedRouteId === route.id ? 'border-blue-300 bg-blue-50' : 'border-gray-200'
        ]"
      >
        <div class="flex items-start justify-between">
          <div class="flex items-center">
            ...
            <div class="ml-3">
              <div class="text-sm font-medium text-gray-900">{{ route.name }}</div>
              <div class="text-xs text-gray-500">{{ getModelName(route.model_used) }}</div>
            </div>
          </div>
          <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="getModelBadgeClass(route.model_used)">
            {{ getModelName(route.model_used) }}
        ...
        <div class="mt-4 grid grid-cols-3 gap-2">
          <div class="text-center">
            <div class="text-sm font-medium text-gray-900">{{ route.total_distance_km.toFixed(1) }} км</div>
            <div class="text-xs text-gray-500">Расстояние</div>
          </div>
          <div class="text-center">
            <div class="text-sm font-medium text-gray-900">{{ route.total_time_hours.toFixed(1) }} ч</div>
            <div class="text-xs text-gray-500">Время</div>
          </div>
          <div class="text-center">
            <div class="text-sm font-medium text-gray-900">{{ route.total_cost_rub.toFixed(0) }} ₽</div>
    ...
    <div v-if="routes.length === 0" class="text-center py-8">
      ...
      <p class="mt-1 text-sm text-gray-500">Создайте первый маршрут в разделе Optimize</p>
    </div>
  </div>
</template>
