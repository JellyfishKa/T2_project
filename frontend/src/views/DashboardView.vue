        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-semibold text-gray-900">{{ routes.total || 0 }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-semibold text-gray-900">{{ averageTime }} ч</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-semibold text-gray-900">{{ averageCost }} ₽</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="lg:col-span-2">
          <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
              <h3 class="text-lg font-semibold text-gray-900">Оптимизированные маршруты</h3>
              <p class="mt-1 text-sm text-gray-600">Список всех созданных маршрутов</p>
            </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-6 py-5 sm:px-7 border-b border-gray-200">
              <h3 class="text-lg font-semibold text-gray-900">Метрики маршрута</h3>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
              <h3 class="text-lg font-semibold text-gray-900">Сравнение моделей</h3>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
          <div class="flex justify-between items-center">
            <div>
              <h3 class="text-lg font-semibold text-gray-900">Последние метрики</h3>
              <p class="mt-1 text-sm text-gray-600">История выполнения оптимизаций</p>
            </div>
            <button 
              @click="loadMetrics"
...
const averageTime = computed(() => {
  ...
  const sum = routes.value.items.reduce((acc, route) => acc + route.total_time_hours, 0)
  return (sum / routes.value.items.length).toFixed(1)
})

const averageCost = computed(() => {
  if (!routes.value.items.length) return 0
  const sum = routes.value.items.reduce((acc, route) => acc + route.total_cost_rub, 0)
