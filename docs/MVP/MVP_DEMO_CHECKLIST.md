...
|-----------|--------|-------------|
| **Frontend (Vue 3 + TypeScript)** | ✅ Готов | SPA: Home, Dashboard, Optimize, Analytics, Schedule, Database |
| **Backend (FastAPI + Python)** | ✅ Готов | Актуальные endpoint'ы в Swagger (`/docs`) |
| **PostgreSQL** | ✅ Готов | 13 таблиц, миграции, ORM-модели |
| **Docker Compose** | ✅ Готов | Multi-stage builds, 3 сервиса |
...
| **Excel-экспорт** | ✅ Готов | openpyxl, 5 листов: Расписание / Журнал / Статистика / Активность ТП / Журнал изменений |
| **State machine визитов** | ✅ Готов | Защита от незаконных переходов статуса (422 на invalid transition) |
| **AuditLog** | ✅ Готов | Журнал: смена статуса, форс-мажор, генерация расписания |
| **Health check v2** | ✅ Готов | disk_free_mb, visits_today, version в ответе /health |
...
| GET | `/api/v1/export/schedule` | Экспорт в Excel: 5 листов с аналитикой и журналом изменений | ✅ |
...
> **Итого:** актуальный список endpoint'ов — в Swagger (`/docs`).
...
| **Database** | `/database` | ✅ | Работа с сущностями БД, вкладки сотрудников и авто |
...
| 3 | `/api/v1/optimize/variants` timeout = 180 сек | При зависании фронт покажет ошибку с кнопкой «Повторить» |
| 4 | CORS по умолчанию — только localhost и 100.120.184.98 | При смене IP обновить `CORS_ORIGINS` в `.env` |
| 5 | LLM не участвует в `POST /api/v1/optimize` (чистый greedy) | Для LLM-функций использовать `/api/v1/optimize/variants` |
...
- [ ] **Тест Excel:** нажать кнопку «Excel» в Schedule — файл скачивается, 5 листов корректны
- [ ] Health check: `curl http://localhost:8000/health` → проверить наличие `disk_free_mb > 1000`, `visits_today`, `version: "1.2.0"`
...
### Рекомендуемые демо-сценарии v1.2

- **State machine**: открыть любой выполненный визит в Schedule → попытаться изменить статус на «Запланирован» → показать ошибку 422
- **Warning**: перевести сотрудника в `sick` → показать `warning` с количеством незакрытых визитов в ответе
- **Excel (5 листов)**: скачать отчёт → показать 5-й лист «Журнал изменений» с записями AuditLog
- **Health v2**: `GET /health` → показать `disk_free_mb`, `visits_today`, `version`
- **Дублирование**: вызвать `POST /schedule/generate` дважды → второй вызов вернёт 409 с `existing_count`
...
3. **Schedule** → открыть месяц, кликнуть на день → Day modal
   ...
   - Скачать Excel-отчёт → открыть 6 листов
4. **Database** → показать вкладки сотрудников и автомобилей
