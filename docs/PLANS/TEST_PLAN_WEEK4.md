...
| Excel экспорт | 6 листов: расписание, журнал, статистика, активность, журнал изменений, маршруты навигатор |
| Excel импорт | Загрузка заполненного файла, обновление БД |
| LLM варианты | `POST /api/v1/optimize/variants` — 3 варианта + pros/cons |
| Сохранение варианта | `POST /api/v1/optimize/confirm` → БД |
...
- **Метод**: `PATCH /api/v1/schedule/{visit_id}`
...
- **Описание**: Excel содержит 6 листов
- **Проверка**: `openpyxl.load_workbook(response.content).sheetnames == ["Расписание", "Журнал визитов", "Статистика по ТТ", "Активность ТП", "Журнал изменений", "Маршруты навигатор"]`
...
| TC-EXCEL-002 | Открыть скачанный файл | 6 листов: Расписание, Журнал, Статистика, Активность, Журнал изменений, Маршруты навигатор |
