...
T2 Розничная сеть — это комплексная платформа на основе искусственного интеллекта для оптимизации маршрутов магазинов и сетевой аналитики с использованием нескольких LLM-моделей (Qwen, T-Pro, Llama).
...
- [x] Интегрированы 3 LLM-модели (Qwen, T-Pro, Llama)
...
- FastAPI (REST API)
- SQLAlchemy (ORM)
- Qwen, T-Pro, Llama (LLM-модели, GGUF через llama-cpp-python)
- Pydantic (валидация)
...
│   │   │   ├── tpro_client.py (вторичная модель)
...
### 1. Поддержка нескольких LLM-моделей
- **Qwen** (основная модель, лучшее качество оптимизации)
- **T-Pro** (вторичная модель, быстрый отклик)
- **Llama** (fallback модель, open-source, максимальная надежность)

Все модели загружаются в формате GGUF через llama-cpp-python. Автоматический fallback при недоступности модели.
...
TRY: Qwen (Primary — лучшее качество)
  ...
  |-- Ошибка -> Следующая модель
    |
TRY: T-Pro (Secondary — быстрый отклик)
  |-- Успех -> Возвращаем результат (model_used="T-Pro")
  |-- Ошибка -> Следующая модель
    |
...
| POST | /tpro/optimize | T-Pro | Оптимизация маршрута (быстрая) |
...
- [x] Получение доступа к LLM (Qwen, T-Pro, Llama)
...
- [x] Все 3 LLM-модели интегрированы
- [ ] MVP готов к демо
...
| T-Pro-it-1.0 | ~64 GB (bf16) / ~32 GB (int8) | ~8 GB | Рекомендуется квантование или vLLM |
